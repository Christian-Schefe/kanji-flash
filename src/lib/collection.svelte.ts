...
export const properKanjiCollection: KanjiCollection = {
	...
	name: "Proper Kanji",
	description:
		"All kanji in the database that have an assigned meaning and at least one reading.",
	contains: (kanji: Kanji) => {
		return (
			kanji.meanings.length > 0 &&
			(kanji.kunReadings.length > 0 || kanji.onReadings.length > 0)
		);
	},
};
...
export const collections: KanjiCollection[] = [
	...
	properKanjiCollection,
	{
		...
		name: "Jouyou Kanji",
		description:
			"The 2136 jouyou kanji, as defined by the Japanese government.",
		contains: (kanji: Kanji) => {
