...
export const storagePrefix = "kanji-flash/";
...
export class LocalStore<T> {
	...
	constructor(
		...
		if (this.checkQuery !== "none" && page.url.searchParams.has(this.key)) {
			...
			const deserializedValue =
				queryItem !== null ? this.tryDeserializeFromURL(queryItem) : null;
		...
		if (browser && !hasQueryValue) {
			const item = localStorage.getItem(storagePrefix + this.key);
			if (item) {
	...
	mount() {
		...
		$effect(() => {
			localStorage.setItem(storagePrefix + this.key, this.serialize(this.value));
