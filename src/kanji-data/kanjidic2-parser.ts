...
const extractKanji = (char: any): Kanji => {
  ...
  return {
    ...
    r: null,
    m: meanings.filter((m) => !m.m_lang || m.m_lang === 'en').map((m) => m['#text'] || m),
    k: readings.filter((r) => r.r_type === 'ja_kun').map((r) => r['#text'] || r),
    o: readings.filter((r) => r.r_type === 'ja_on').map((r) => r['#text'] || r)
  };
};
