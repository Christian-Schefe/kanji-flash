<script lang="ts">
  ...
  const jlpt = ['N0', 'N1', 'N2 - N3', 'N4', 'N5'][data.kanji?.j ?? 0];
  const grade = [
    'Ungraded',
    'Kyouiku',
    'Kyouiku',
      ...
      <Card size="md">
        <div class="grid" style="grid-template-columns: min-content 1fr;">
          ...
          <div class="grid col-span-2" style="grid-template-columns: min-content 1fr;">
            ...
            <div class="flex flex-wrap">
              {#each data.kanji.k as reading, index}
                {#if index > 0}
                  <p class="mr-2">,</p>
                {/if}
                <p class={fontClass}>{reading}</p>
